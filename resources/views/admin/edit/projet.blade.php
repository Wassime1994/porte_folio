...
<div class="md:p-12 bg-indigo-100 flex flex-row w-screen h-screen">
    <form action={{route('projet.update' , $data->id)}} method="POST"class="md:w-1/2-screen m-0 p-5 bg-white w-full tw-h-full shadow md:rounded-lg">
        @csrf
        @method('PUT')
      ...
      <div class="flex-col flex py-3">
        <label class="pb-2 text-gray-700 font-semibold">Language</label>
        <input value={{$data->language}} name="language" type="text" class="p-2 shadow rounded-lg bg-gray-100 outline-none focus:bg-gray-200" >
      </div>
